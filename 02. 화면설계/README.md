...
- UI 유형
    ...
    - NUI (Natural User Interface)
        - 직관적 사용자 반응 기반 인터페이스
        - 신체부위를 이용하는 사용자 인터페이스 (터치, 음성 등)
    - OUI (Organic User Interface)
        - 유기적 상호작용 기반 인터페이스
        - 현실에 존재하는 모든 사물이 입출력장치로 변화할 수 있는 사용자 인터페이스
    
- UI 설계 원칙
    - **원칙 보고 설명 작성, 설명 보고 정의 작성**
    - 직관성
        - 누구나 쉽게 이해하고, 쉽게 사용할 수 있어야한다.
    - 유효성
        - 정확하고 완벽하게 사용자의 목표가 달성될 수 있도록 제작
    - 학습성
        - 초보와 숙련자 모두가 쉽게 배우고 사용할 수 있게 제작
...
- UI 설계 지침
    - **설명 보고 지침 적기**
    - 사용자 중심
        - 사용자가 이해하기 쉽고 편하게 사용할 수 있는 환경 제공, 실사용자에 대한 이해가 바탕이 되어야함.
    - 일관성
        - 버튼이나 조작 방법을 사용자가 기억하기 쉽고, 빠르게 습득할 수 있도록 설계
    - 단순성
        - 조작방법은 가장 간단하게 작동되도록 하여 인지적 부담 최소화
    - 가시성
        - 주요 기능을 메인에 노출하여 쉬운 조작이 가능해야 함
    - 표준화
        - 디자인을 표준화하여 기능구조의 선행학습 이후 쉽게 사용 가능해야함
    - 접근성
...
- UI 품질 요구사항
    ...
    - 기능성
        - 적절성
            - 소프트웨어 제품이 주어진 작업과 사용자의 목표에 필요 적절한 기능들을 제공해줄 수 있는 소프트웨어의 능력
        - 정밀성
    ...
    - 사용성
        - 이해성
            - 소프트웨어의 논리적 개념과 적용 가능성을 구분하는데 필요한 사용자의 노력 정도에 따른 소프트웨어 특성
        - 학습성
...
- UI 개발을 위한 주요 기법
    - 3C분석
        - 고객, 자사, 경쟁사를 비교하고 분석하여 자사를 어떻게 차별화해서 경쟁에서 이길 것인가를 분석하는 기법
    - SWOT 분석
        - 기업의 내부 환경과 외부 환경을 분석하여 강점, 약점,기회, 위협 요인을 규정하고, 이를 토대로 경영 전략을 수립하는 방법
    - 시나리오 플래닝
    - 사용성 테스트
        - 사용자가 직접 제품을 사용하며 작성된 시나리오에 맞춰 과제를 수행한 후, 질문에 답하도록 하는 테스트
        
- 스토리보드 개념
    - 디자이너와 개발자가 최종적으로 참고하는 산출 문서
    - 정책, 프로세스, 콘텐츠 구성, 기능 정의, DB 연동 등 구축하는 서비스를 위한 정보가 수록된 문서
    - UI 화면 설계를 위해 와이어프레임, 스토리보드, 프로토타입이 활용됨.

- UI 화면 설계
    - 와이어프레임
        - 이해 관계자들과의 화면구성을 협의하거나 서비스의 간략한 흐름을 공유하기 위해 화면 단위의 레이아웃을 설계하는 작업
    - 스토리보드
        - 정책, 프로세스, 콘텐츠 구성, 와이어프레임, 기능 정의, DB 연동 등 서비스 구축을 위한 모든 정보가 담겨있는 설계 산출물
    - 프로토타입
...
- UML 유형
    - **설명 보고 유형 적기**
    - 클래스 다이어그램
        - 객체지향 모델링 시 클래스의 속성 및 연관과 클래스 간 정적인 관계를 표현한 다이어그램
        - 클래스, 속성, 연산, 접근제어자, 관계
        - 클래스 간 관계도 암기해야함(다음 암기문제)
    ...
    - 시퀀스 다이어그램
        ...
        - 객체 간 동적 상호작용을 시간적 개념을 중심으로 모델링하는 과정
        - 액터, 객체, 생명선, 활성화, 메시지
    ...
    - 활동 다이어그램
        ...
        - 유스케이스안 또는 사이에서 발생한 복잡한 처리의 흐름을 명확하게 표현 가능
        - 시작점, 전이, 액션, 노드 등
    ...
    - 커뮤니케이션 다이어그램  
        - 동작에 참여하는 객체들이 주고받는 메시지를 표현하고, 객체 간 연관까지 표현하는 다이어그램
        - 상호작용하는 과정을 표현
        
    - 컴포넌트 다이어그래    
        - 시스템을 구성하는 물리적인 컴포넌트와 그 사이 의존 관계를 나타내는 다이어그램
        - 실질적 프로그래밍 작업에 사용
        
- 클래스 간의 관계
    - 연관 관계
        - 서로 개념적으로 연결된 선 (실선)
        - 2개 이상의 사물이 서로 관련되어 있는 상태 표현
    
    - 집합관계
        - 하나의 객체에 여러 독립 객체들이 구성되는 관계(빈 마름모)
        
    - 복합 관계
        - 영구적이고, 집합 관계보다 더 강한 관계로 구성 (채워진 마름모)
    
    - 일반화 관계
        - 하나의 사물이 다른 사물에 비해 일반적인지 구체적인지 표현
        - 일반적 = 부모, 구체적 = 자식(하위)
        - 일반적 쪽으로 빈 화살표로 연결 (내 부모는 얘야)
...
- UI 흐름 설계
    - 개념
    - 업무관련 화면의 위치와 흐름을 흐름도 형식으로 표현하는 활동
...
- UI 시나리오 문서 작성요건
    ...
    - 완전성
        ...
        - 사용자 태스크에 초점
    - 일관성
        - 요구사항의 일관성, UI 스타일을 일관적으로 구성
    - 이해성
        - 이해하기 쉽도록 구성 및 설명
        - 추상적, 전문적 용어 사용하지 말아야함
    - 가독성
        - 표준화된 템플릿 작성
        - 버전 넘버링의 일관성, 하이라이팅의 일관성
    - 추적 용이성
        - 변경 시점, 이유 등을 규명 -> 추적하기 쉽게
    - 수정 용이성
        - 쉽게 변경 가능, 시나리오에 쉬운 반영
        - 동일한 수정사항을 위해 여러 문서를 편집하지 않도록
